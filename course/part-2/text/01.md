# 2.01 - Custom Value Object and QObject
...
The identity type derives from `QObject`. It extends C++ with many dynamic features using a meta-object system. It cannot be copied as the copy constructor and assignment operator equal to delete. Examples include: `QWidget`, `QWindow`, `QTcpSocket`, and so on.

### Value Type vs Identity Type

// This is discussed briefly at the end of the Strings chapter too, but I don't think the repition is necessarily bad. I might cut it down to be a bit shorter though. -Jan

Features of Qt Objects require us to treat them as identities, not values. Values are copied or assigned; identities are cloned. Cloning means to create a new identity, not an exact copy of the old one. For example, twins have different identities. They may look identical, but they have different names, different locations, and may have completely different social networks.
...
For these reasons `QObject` and all subclasses of `QObject` (direct or indirect) have their copy constructor and assignment operator disabled.

## Custom Value Objects

// Brief explanation about Q_DECLARE_METATYPE and its uses here, before going on to QObjects. 

Before going on to QObjects, let's look at how the Qt property system provided by the Qt meta-object system can be used with custom value objects. Custom types used by properties need to be registered using the `Q_DECLARE_METATYPE()` macro so that their values can be stored in `QVariant` objects. This makes them suitable for use with both static properties declared using the `Q_PROPERTY()` macro in class definitions and dynamic properties created at run-time. We will discuss the property system later in the chapter.
...
// Remove this section, explained at the start.
