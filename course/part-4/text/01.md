...
Add an action to menu to add and remove images from the model. The add image action should open a file dialog, where the user can choose multiple image files to be added to the model. The remove action should remove the last image from the model. Do not try to remove any image from an empty model.
...
Add yet another action to the menu. When triggered, the action should add 10,000 copies of one of the model fileUrls to the model. Obviously, the UI must be kept responsive all the time. 
