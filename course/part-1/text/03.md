...
1) `addStudent` to add students in the `studentMap` container. Use `int` studentNumber as the key in the container.
2) `findByCredits` that iterates over the container and returns a list of all names who have >= credits.
3) `removeDivisibleStudents` that removes all students with a student number that is divisible by N from the container.
4) `countStudentsStartingWith` that counts the amount of students with a name starting with char n.
